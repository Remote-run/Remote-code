...
public class ContainerStateManager {
    ...
    private static final String FIND_LEAST_POPULATED_NODE = "SELECT computenode.id, count(computenode.id)\n" + "FROM computenode \n" + String
            .format("left join (SELECT * from container where container.containerstatus='%s') c\n",
    ...
    public void RequestNewState(Project project, ContainerStatus newState) {
        ...
        newTask.setAction(ContainerAction.START);
